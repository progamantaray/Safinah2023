...
def write_json(data, filename='/home/safinahonejuara1kkctbn2021/Documents/safinah_vision-master/core/data.json'): 
...
def draw_bbox(image, bboxes, info = False, counted_classes = None, show_label=True, allowed_classes=list(read_class_names(cfg.YOLO.CLASSES).values()), read_plate = False):
    ...
    for i in range(num_boxes):
        ...
        if class_name not in allowed_classes:
            ...
            if (class_ind == 0):
                ...
                cv2.rectangle(image, c1, (np.float32(c3[0]), np.float32(c3[1])), (0, 255, 0), -1) #filled
                ...
                t_size = cv2.getTextSize(bbox_mess, 0, fontScale, thickness=bbox_thick // 2)[0]
                c3 = (c1[0] + t_size[0], c1[1] - t_size[1] - 3)
                cv2.rectangle(image, c1, (np.float32(c3[0]), np.float32(c3[1])), (255, 0, 0), -1) #filled
    ...
    if (len(temp_merah) > 0 and len(temp_hijau) > 0):
        cv2.line(image, (temp_max_merah[4], temp_max_merah[1]), (int(image_w/2), int(image_h/2)), (255, 0, 0), thickness=3)
        cv2.line(image, (temp_max_hijau[2], temp_max_hijau[1]), (int(image_w/2), int(image_h/2)), (0, 255, 0), thickness=3)

    with open('/home/safinahonejuara1kkctbn2021/Documents/safinah_vision-master/core/data.json') as json_file:
