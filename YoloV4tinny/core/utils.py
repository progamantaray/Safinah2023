...
def write_json(data, filename='D:\GAMANTARAY\safinah_vision-master\core\data.json'): 
...
def draw_bbox(image, bboxes, info = False, counted_classes = None, show_label=True, allowed_classes=list(read_class_names(cfg.YOLO.CLASSES).values()), read_plate = False):
    #Kode Waba
    cv2.rectangle(image, (0,0), (image.shape[1],100), (0,0,255), thickness=-1)
    cv2.line(image, (image.shape[1]//5,50), (-5,85), (0,0,255), thickness=105) ###
    cv2.line(image, (image.shape[1]-image.shape[1]//5,50), (image.shape[1]+5,85), (0,0,255), thickness=105)
    centroid_dict = dict()
            ...
            if info == False:
                ...
                if (class_ind == 0):
                    ...
                    temp_hijau[centroid_dict[i]]=centroid_dict[i][1]
                     ##
                    ##gabisaa print(temp_hijau[centroid_dict[i]]=centroid_dict[i][1])
                     ##
                    # deteksi = {
...
###ALAMAT FILE DISESUAIKAN KE data.json PADA KOMPUTER
    with open('D:\GAMANTARAY\safinah_vision-master\core\data.json') as json_file:
